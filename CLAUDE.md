...
| **루트 액세스 키 폐기** | 🟠 **서버 무관 확정(2026-08-09 실측)** — 08-08 의 "운영이 루트 키로 돈다"는 **오경보**였다. 서버 키 끝 4자 `VHTO` = `bideasy-ses-sender` 의 키(`…BXS3VHTO`)이고, 진짜 루트 키(`…LAO73L7F`)는 마지막 사용이 08-07 `aws-mcp`(us-east-1)로 SES 가 아니다. 오판 원인 = 같은 계정 키의 공통 접두사(`AKIAY5OAWO54`) 14자만 보고 단정. **다만 루트 키는 여전히 Active** 이고 **맥북 `~/.aws/credentials` [default] 에 평문** + aws-mcp 도구가 사용 중이다. 순서 = **소비자(aws-mcp)를 최소 권한 IAM 키로 이전 → 루트 키 비활성화 → 2~3일 관찰 → 삭제 + 루트 MFA 확인**. 절차 = `docs/SECRET_ROTATION.md` §3-1(08-09 재정정) |
