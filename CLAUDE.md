...
> 최종 갱신: 2026-08-10 (**모의투찰 재점검·잔여 구현 — 로컬 변경, 미배포. pytest 844건·Ruff·JS/JSON 검증 통과.** ① 과거 문서의 “SCORED 2,275건으로 G2 400건 충족”은 **arm 행을 공고 표본으로 오독한 결론**이었다. 운영 재실측은 8,865 arm 행 = 1,773공고, 확정 채점 496공고, G-A **27.98%**(마감 도래 코호트 28.72%)다. 더구나 확정 496공고 중 기초금액 일관성 통과는 **59공고**, 적격심사제는 **2공고**뿐이라 성능 해석과 G2 재판정은 아직 금지다. ② 원인은 채점 큐도 있었다 — 오래된 NO_RESULT 991공고×5 arm이 5,000행 limit의 4,955행을 매일 재점유해 신규·결과도착분을 굶겼다. 우선순위를 **결과 도착 → 최초 확인 → NO_RESULT 재시도**로 고치고 큐 분해 지표를 추가했다. ③ 성능 집계 전부에 기초금액 일관성 가드(0.94~1.06)와 제외 수를 적용하고, G-A/G-B/G-C를 distinct 공고·Wilson 95% CI로 자동 판정해 미달 화면을 잠근다. ④ `mock_bid.weekly_report`(월 21:00)와 2026 G2 CLI(`--include-db --holdout-year 2026`)를 구현했다. 비정본 실행은 운영 frontier JSON을 덮어쓸 수 없다. ⑤ 새 IDE 이관 문제는 workspace가 Windows `Scripts/python.exe`를 가리킨 것이 원인 — `backend/.venv`의 OS 중립 인터프리터와 macOS/Windows별 task로 수정. **운영 복구는 이 변경을 배포한 뒤 관찰해야 하며 아직 실행하지 않았다.** 정본 = `docs/MOCK_BIDDING_DESIGN.md` §9.)
...
```bash
cd backend && pytest          # 844건 통과 기준 (2026-08-10 로컬 변경 실측. SQLite 파일)
