...
> 최종 갱신(2026-08-16): **블로그 K-트랙 자동 포스팅 Phase 2 완성 — 로컬, 미push·미배포.** `feat/blog-knowledge-autopublish`를 최신 master `b130563` 위로 rebase하고, 수요일 07:00 자동초안이 **검사가 생략되지 않은 PASS/WARN**일 때만 기본 48시간 유예 예약되도록 고정했다. LLM 심판 실패(`skipped`)·FAIL·검수 없음은 사람 호출, 매시 발행 직전에도 판정을 재확인해 불완전 예약을 해제한다. 자동 글의 제목/본문 수정·force 재생성은 옛 판정·채널 자산·예약을 함께 무효화한다. 예약과 사전 알림은 한 트랜잭션이며, due 행은 Postgres `FOR UPDATE SKIP LOCKED`로 선점한다. 히어로 404/확인 실패는 깨진 URL 대신 이미지 없이 발행하고 복구 대상을 알린다. 운영 읽기 전용 기준 = K1~K8 published, K9 FAIL draft, 예약 0 → 배포 후 다음 주간 루프 대상은 K10. 검증 = backend **979 passed**·Ruff clean·블로그 관련 **141 passed**·diff check clean. push·PR·배포·LLM 실호출·콘텐츠 발행은 하지 않았다.
