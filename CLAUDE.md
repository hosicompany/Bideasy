...
16. **가입·진단 폼은 이메일 소유를 확인하지 않는다** — `/signup`·`/leads/capture` 둘 다 체크박스만으로는 광고 대상이 되지 않는다(`grant_marketing(confirmed=False)` → 확인 링크 클릭). 소셜 로그인만 제공자가 `email_verified` 를 준다. 새 가입 경로를 만들 때 이 계약을 빠뜨리면 제3자 주소로 광고가 나간다.
...
```bash
cd backend && pytest          # 557건 통과 기준 (2026-08-01 master 실측, PR #50 병합분 포함. SQLite in-memory/파일)
