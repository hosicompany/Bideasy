...
| 모의투찰 | REGISTERED 5,680 / SCORED **2,275** | G2 재판정 조건(2026 개찰 400건+)은 **이미 넘겼다** — `docs/BENCHMARK_WIN_REACH.md` 재판정 착수 가능 |
...
- **G2 재판정 (조건 충족됨)** — 모의투찰 SCORED 2,275 건으로 `docs/BENCHMARK_WIN_REACH.md` 의 "2026 개찰 400건+ 누적" 조건을 넘겼다. **다만 표본에 기초금액 결함(함정 22) 이전 데이터가 섞여 있는지 먼저 볼 것** — `arm_backtest.base_is_consistent` 가드가 있으나 재판정 전에 확인이 필요하다.
...
```bash
cd backend && pytest          # 824건 통과 기준 (2026-08-08 master 99199f9 실측, 약 2분. SQLite in-memory/파일)
