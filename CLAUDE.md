...
> 그 전: 2026-08-01 (**리드 육성 시퀀스 라이브** — PR #50, E2E 실검증. 진단 캡처 → 확인 메일 → 웰컴 → 매주 화 08:00 신규 매칭. 설계 판단 5가지 = `docs/LEAD_ACQUISITION.md` §3-3: 자격 판정 단일 소스(`lead_matching.py`) · 확인 전 광고 금지 · 멱등 주체는 **행이 아니라 수신자** · `can_send_marketing` 폴백 제거 · 실패는 1건에 갇힌다.)
