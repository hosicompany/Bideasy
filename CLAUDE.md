...
> 최종 갱신(2026-08-17): **모의투찰 가상 등수 fail-closed + 최종 참가자 스냅샷 무결성 수습 — 미배포.** 운영 실측은 G-A **55.22%(1,699/3,077공고, OBSERVING)**·G-B 유효 적격 **91/400(BLOCKED_G_A)**이고, 최근 300공고 참가자 140,801행 중 API 등수와 가격 재계산 등수가 다른 행이 **12,831건(17.708%, 56공고)**이었다. 불일치는 전부 API 등수보다 재계산 등수가 뒤로 밀린 방향이며, 원인은 진행 중 응답을 누적 merge한 뒤 최종 응답에서 사라진 옛 참가자를 삭제하지 않아 생긴 유령 행이다. 수정은 ① `axis_health.healthy === true`가 아니면 요약·차트뿐 아니라 공고별 이력의 등수도 숨기는 fail-closed ② 진행 중에는 merge, 낙찰자 확정 뒤에는 응답 내부 등수축 오차 1% 미만인 스냅샷만 원자적으로 동기화 ③ 기본 dry-run 복구 CLI 추가다. 검증 = backend **988 passed**·Ruff clean·관리자 JS 구문·CLI help·diff check 통과. 정본 상세 = `docs/MOCK_BIDDING_DESIGN.md` §9.
...
> 직전 갱신(2026-08-15): **음성 전 프리프로덕션과 비공개 creative 입력 경로까지 완료 — 여전히 로컬 변경, 미커밋·미배포·유료 생성 없음.** 공식 완료 전기공사 공고 `R26BK01488342-000`을 이해도 시연 사례로 고정하고, A/B는 0~2초 훅만 다르며 2~15초는 동일한 1080×1920·15초 무음 PREVIZ와 provider용 무문자 storyboard를 결정적으로 생성한다. 실제 source UI는 `0~2초 숨김 → 2~5초 등장 → 5~13초 결과 확대 → 13~15초 숨김`으로 합성하며, 기존 비전기공사·구형·개인정보 포함 화면 5장은 `campaign_approved=false`로 생성 입력에서 차단했다. 새 source UI·storyboard·대표 음성은 관리자 인증 multipart로 `_inputs/{creative_id}`에만 보관하고, DB 원장·SHA·역할·MIME을 재검증한 뒤 정확한 runner 전용 URL에만 회전 서비스 토큰을 붙인다. 손상·잘림 WAV는 실제 PCM 프레임을 끝까지 확인해 거부하며, 업로드 도중 브리프가 바뀌어도 다른 폼에 manifest를 연결하지 않는다. 업로드 후 저장되지 않은 manifest는 목록·재연결로 복구하며 자동 저장·승인·queue·publish는 하지 않는다. 검증 = backend **1,021 passed**·Ruff clean·Alembic single head `a6d9c3e71b42`, runner **73 passed**, Extension **54 passed**·production build·release ZIP(20 files, 78 KB)·production dependency audit 0. 브라우저 연결과 Docker/Nginx 실행 파일이 없어 새 현행 G2B+BidEasy 실제 화면 캡처·브라우저 업로드 E2E·`nginx -t`·PostgreSQL 2-session 경합 실증은 미수행이다. ⚠️ 남은 생성 게이트는 마스킹·검수된 새 전기공사 source UI, 대표의 12.5~14.8초 실제 음성, 사람 승인이다. 모집·Higgsfield 유료 생성·게시·배포는 모두 보류하며, Extension v1.2 제출 전 실제 `bideasy.kr/privacy` 정책 갱신·기존 사용자 고지가 필요하다.
> 그 전(2026-08-14): **온라인 메시지 검증·Higgsfield 로컬 creative runner 구현 완료 — 로컬 변경, 미커밋·미배포·유료 생성 없음.** 외부 획득 메시지를 `나라장터 공고 옆에서, 자격·A값·하한선을 한 번에.`, 엔드라인을 `투찰 전 마지막 확인, BidEasy.`로 정렬하고 A/B 5초 비보조 테스트(허가형 access key, 타깃 선별, 균형 배정, 사전 코딩·게이트)를 추가했다. Creative brief/attempt/output 3테이블과 승인 상태기계, `/admin-creatives`, runner lease·회전형 토큰·append-only 결과·`creative_id` 퍼널 귀속을 구현했다. 인증된 Mac runner는 Higgsfield CLI **1.1.23**만 허용하고 Marketing Studio/Reframe 후 실제 UI·승인 카피·대표 음성을 Pillow/FFmpeg로 결정적 합성하며, 완성 MP4만 Virality Predictor에 보내고 별도 job ID로 재결합한다. provider 원문·자격증명은 서버/공개 보고서에 저장하지 않는다. Extension v1.2는 로그인 상태의 공개 `bid_no` 확인만 중복 제거해 활성화로 전송하며 URL·공고명·회사 프로필·creative/UTM은 전송하지 않는다. 검증 = backend **999 passed**·Ruff clean·Alembic single head `f1c7a4e82d90`, runner **60 passed**·비과금 live preflight 통과, Extension **50 passed**·build/release ZIP 검수·production dependency audit 0, 정적 JS/diff check 통과. ⚠️ 모집은 기본 OFF이며 승인된 마스킹 전기공사 화면+캡션 설정 전 켤 수 없다. Extension v1.2 제출 전 실제 `bideasy.kr/privacy` 정책 갱신·기존 사용자 고지가 필요하고, 게시·배포·외부 모집은 사람 승인 뒤 별도 실행한다.
> 그보다 전(2026-08-11): **PR #105 후속 UI 경쟁 조건 수습 + 승인 강제 공백 확인.** PR #105는 master `2173cc0`으로 병합·운영 배포됐다. 독립 리뷰에서 관리자 이력 필터를 빠르게 바꾸면 늦은 이전 응답이 최신 화면을 덮는 문제를 확인해 `fix/mock-bidding-history-request-race`에서 request sequence 가드와 회귀 테스트를 추가했다. 검증 = pytest **874 passed**·Ruff clean·관리자 JS 구문·diff check 통과, push 전 로컬 상태. GitHub 설정은 `master` branch protection 없음, `production` 보호 규칙 0개, admin bypass 허용이다. 동일 `hosicompany` 계정을 사람과 에이전트가 함께 쓰는 한 GitHub 로그도 세션을 구분하지 못하므로, 승인을 기술적으로 강제하려면 별도 reviewer 계정/팀이 필요하다. 설정은 사람 결정 전 변경하지 않는다.
...
**나라장터 공고 옆에서 참가조건·A값·낙찰하한선·주의 조항을 확인하는 도구.** 크롬 익스텐션 + 웹앱(bideasy.kr) + 공유 FastAPI 백엔드로 구성된 3-Tier SaaS. 외부 획득 메시지는 `나라장터 공고 옆에서, 자격·A값·하한선을 한 번에.`, 브랜드 엔드라인은 `투찰 전 마지막 확인, BidEasy.`다. **낙찰가는 예측하지 않고**, 확인 가능한 공고 기준과 근거를 같은 화면에 보여주는 Chrome 사이드패널 워크플로우에 집중한다.
...
| 채널 | 역할 | 위치 |
|---|---|---|
| **크롬 익스텐션** | 나라장터 공고 옆 사이드패널 — 자격·A값·하한선·주의 조항 확인 | **별도 레포** `Bideasy-Extension/` (이 폴더 아님) |
...
| **Chrome 웹스토어** | ✅ **v1.1.0 심사 통과·라이브(2026-08-17 대시보드 실측)** — 버전 **1.1.0** · 상태 **게시됨-공개** · 최종 업데이트 2026-08-06 · ID `ecdpfnfonccaaneihkdlkhnkpjakmmnp`(문구 정본 = `docs/STORE_LISTING.md`). 📌 **사용자 11명** — 이 숫자가 이번에 처음 관측됐고, 같은 시점 웹 회원 2명·외부 리드 0명이다. **광고비 0원 채널이 유료 검색보다 5배 많이 획득하고 있다**(네이버·구글 양쪽에서 도구 키워드 수요 없음이 실측으로 확인된 것과 대비된다). ⚠️ 11명 중 본인·테스트 설치 수는 미상이라 실사용자는 더 적을 수 있다. 남은 일 = **listing 대시보드 반영**(개정본은 준비돼 있고 수동 작업) |
...
- ~~**익스텐션 `feat/diagnose-cta` 를 `main` 에 병합**~~ → ✅ **완료(2026-08-17)**. 웹스토어 v1.1.0 심사 통과를 대시보드로 확인한 뒤 PR #2 를 squash merge 했다(익스텐션 레포 `main` `198f736` → **`05deefc`**, 브랜치 삭제). **스토어는 이미 이 코드로 라이브였으므로 사용자 영향 0** — 레포 기록을 현실과 맞춘 작업이다. 11일 동안 "스토어엔 나가 있는데 `main` 엔 없는" 상태였고, 그게 이 문서가 경고한 유령 항목 경로다. 검증 = `npm test` **47 passed**·`npm run build` 성공·worktree clean.
...
19. ~~**마이그레이션이 실패해도 코드는 이미 배포돼 있다**~~ → **해소됨(2026-08-13)**. 종전 `deploy.sh` 는 pull → build → 재시작 → `alembic upgrade` 순서라, 마이그레이션만 실패하면 **새 코드 + 구 스키마**라는 최악 조합이 남았다(2026-08-12 사고에서 실제로 `valid_participants_count` 없는 상태로 앱이 떴다). 이제 마이그레이션이 **app 재시작보다 먼저** 돌고, `exec` 가 아니라 `dc run --rm --no-deps app alembic upgrade head` 라 **구 app 이 서비스를 계속하는 동안** 스키마만 올라간다 — 마이그레이션이 실패하면 배포가 거기서 서고 구 코드·구 스키마 조합이 유지된다. 순서 역전은 `tests/test_deploy_script.py` 가 막는다(옛 `exec app alembic upgrade head` 문자열 부재를 강제). 실패 시 대응은 그대로: ① 무거운 단계 직전까지 수동 `alembic upgrade <rev>` 로 맞추고 ② 원인을 고친다.
...
```
`deploy.sh`가 자동 수행: `git pull origin master` → `dc build app celery_worker celery_beat` → **`dc run --rm --no-deps app alembic upgrade head`** → 롤링 재시작(app → celery_worker → nginx reload → celery_beat) → 헬스체크.
- ⚠️ **마이그레이션이 재시작보다 먼저다**(2026-08-13 순서 교정, §함정 19). 새 코드가 새 테이블을 즉시 쓰므로 스키마가 먼저 준비돼야 하고, `run --rm` 이라 마이그레이션이 도는 동안 구 app 이 계속 서비스한다.
