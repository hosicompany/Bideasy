...
| 항목 | 상태 |
|---|---|
| **AWS SES** | ✅ **발송은 가동**(2026-07-30). 프로덕션 승인 GRANTED(50,000통/일·14통/초, ap-northeast-2) + 도메인 Verified·DKIM 3종 SUCCESS·SPF·DMARC + **실발송 검증 완료**. ⚠️ 단 **자격증명이 발송 전용 IAM 이 아니다** — 아래 루트 키 항목 참조 |
| **🚨 루트 액세스 키 교체** | 🔴 **최우선 — 운영 서버가 이 키로 돌고 있다**(2026-08-08 실측). `docker exec bideasy_app` 의 `AWS_ACCESS_KEY_ID` 가 **AWS 루트 계정 액세스 키** `AKIAY5OAWO54BX…` 다. 루트 키는 MFA 로도 권한 축소가 불가능한 전권이라, 유출되면 계정 전체를 잃는다. ⚠️ **먼저 지우면 안 된다** — 체험 만료 고지·수신거부 결과 통지 같은 **법정 고지 메일까지 끊긴다**. 순서는 **발송 전용 IAM 키 발급 → 서버 주입 → 배포·발송 검증 → 그 다음 루트 키 비활성화 → 관찰 → 삭제**. 절차 = `docs/SECRET_ROTATION.md` §3-1. 진행 상황: 2026-08-05 에 사전 백업(`~/env.production.bak-20260805-rootkey`)까지만 하고 중단됨. ⚠️ 2026-08-08 정정: 이전 판(2026-08-01)의 "서버는 `bideasy-ses-sender` 를 쓰므로 로컬 조치만 남았다"는 **사실이 아니었다.** 로컬 PC 는 실제로 무관하지만(aws CLI 미설치·`[bideasy]` 프로필만), **서버가 루트 키를 쓰고 있다**는 걸 아무도 확인하지 않았다 |
...
> ⚠️ **master 에 미배포 커밋 1건** — #90(`99199f9`, 누적 개찰 통계 S1 + 마이그 `f7c4a2e18b53`).
> 운영은 `27439db` / `b6f1d3a48c27`. 아래 수치는 **#90 배포 전** 기준이다.
...
### 다음 주제
- 🚨 **AWS 루트 키 교체 (최우선·보안)** — 운영 서버가 루트 키로 SES 를 부르고 있다. 절차 = `docs/SECRET_ROTATION.md` §3-1. **교체·검증 후 폐기** 순서를 반드시 지킬 것(먼저 지우면 법정 고지 메일까지 끊긴다). 사용자 손이 필요한 부분 = IAM 키 발급 + 서버 주입(비밀값은 세션에 남기지 않는다).
...
4-1. **AWS 자격증명 분리**: 서버(`.env.production`)에는 **발송 전용** `bideasy-ses-sender` 키만 두어야 한다(`ses:SendRawEmail`·`SendEmail`, 서울 리전 한정). 콘솔 관리용 `bideasy-ses-admin`(`ses:*`)을 서버에 두지 말 것. 🚨 **2026-08-08 실측 — 이 규칙은 지금 지켜지지 않고 있다.** 운영 컨테이너의 키는 **루트 계정 액세스 키**(`AKIAY5OAWO54BX…`)다. 교체 전까지 이 문단은 "해야 할 일"이지 "현재 상태"가 아니다(§대기 항목 최상단).
   - 확인 명령(값은 절대 출력하지 말 것 — ID 앞자리·길이만):
