...
- **블로그 이미지 WebP 전환 (착수 대기 — 스코프·실측 완료 2026-08-03)** — PNG 14장(히어로 9·도식 5) 합계 **10.7MB → 0.37MB, 97% 절감**(q85 실측. `tuchalga-calculation/hero.png` 1,258KB→25KB). 독자가 모바일 중심이라 LCP·SEO 직격이고, **이미 라이브인 상록수 4편·K1 이 그대로 무겁다**.
  **권장 = nginx rewrite 방식**: URL 은 `.png` 그대로 두고 `location ~ ^/assets/blog/(.+)\.png$` 에서 `.webp` 를 먼저 찾게 한다 → **코드·마크다운·DB `body_md`/`hero`·테스트 전부 변경 0**(경로를 `.webp` 로 바꾸는 안은 이미 발행된 글의 DB 까지 고쳐야 한다). PNG 는 삭제하고 WebP 만 커밋해 레포도 함께 가벼워진다. 롤백은 nginx 블록 제거.
  전제 확인됨: nginx 1.29.5 · `image/webp` MIME 등재됨. 품질은 **히어로 q82 / 도식 q92**(한글 라벨 보존) 로 시작하고 **도식 5장은 눈검수 필수**. 리스크 = WebP 미지원 구형 브라우저(2026 기준 무시 가능, 폴백 원하면 PNG 존치).
...
20. **힉스필드 CLI 는 워크스페이스 선택이 안 되면 전부 실패한다** — 인증 토큰이 멀쩡해도 `generate` 계열이 "No workspace selected" 로 죽는다(`higgsfield workspace set <id>`, 설정은 유지됨). ⚠️ 에러 힌트가 `hf ...` 라고 안내하는데 **이 PC의 `hf` 는 huggingface_hub** 이니 `higgsfield`/`higgs` 를 쓸 것. 생성물은 2752x1536·4MB 로 나오므로 **리사이즈(1376x768)+양자화가 필수**(독자가 모바일 중심). ⚠️ 그래도 PNG 는 사진형에 최악이라 장당 450KB~1.3MB 다 — **WebP 로 저장할 것**(같은 그림이 q85 에서 25KB, 97% 절감. 2026-08-03 실측).
