...
- **검수 게이트 관측 → Phase 2 판단** — 배포·LLM 키·실호출은 2026-08-03 로 **끝났다**(위 대기표). 남은 건 표본 수집뿐: `/admin-blog` 에서 K-큐 초안을 만들며 **검수 판정과 본인 판단이 맞는지** 눈으로 대조한다. 수 주 뒤 `GET /admin/blog/review-stats` 로 일치율을 보고 **Phase 2(판정 연동 자동발행)** 착수 여부 판단 — 거짓경보(FAIL×published)가 0 에 수렴해야 한다. 발행 시 도식은 본문의 `<!-- 이미지 자리 -->` 주석을 사람이 해제해야 뜬다(히어로는 자동).
- **블로그 이미지 WebP 배포 확인 (2026-08-03 구현 완료, 배포만 남음)** — 배포 후 `curl -H "Accept: image/webp" -I https://bideasy.kr/assets/blog/knowledge-k1/hero.png` 가 `content-type: image/webp` 를, `Accept: */*` 는 `image/png` 를 주는지 확인한다. 실패해도 PNG 로 폴백되므로 사고는 나지 않는다.
