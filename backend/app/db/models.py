...
class OpeningParticipant(Base):
    ...
    참가자에게 동순위를 줄 가능성을 배제할 수 없다. 중복은 대신
    `(bid_no, company, bid_price)` UNIQUE 가 막는다(마이그 `c9a4e7b13f28`).

    ⚠️ 재크롤은 **진행 중 병합, 낙찰자 확정 후 최종 스냅샷 동기화**다
    (`_save_participants`). 부분 응답은 기존 행을 지우지 않지만, 확정 후에도
    계속 병합하면 서로 다른 시점의 `opengRank`가 섞여 가상 순위가 뒤로
    밀린다(2026-08-17 운영 실측 17.708%). 확정 응답의 순위 축을 검증한 뒤
    공고 단위로 동기화해 유령 행을 제거한다. `company` 는 파서에서
    `.strip()` 으로 정규화하고 UNIQUE 를 건다.
    """
...
class BlogPost(Base):
    ...
    # 자동 검수 게이트 판정 (services/content_review.py). 판정 생성 자체는 저장만 하고,
    # K-트랙 주간 태스크가 Phase 2 라우팅(PASS/WARN 예약·FAIL/미완료 사람 호출)에 사용한다.
