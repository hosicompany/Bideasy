...
class OpeningParticipant(Base):
    ...
    ⚠️ 재크롤은 **병합**이다 — 삭제하지 않는다(`_save_participants`).
    삭제-재삽입은 부분 응답이 완전 집합을 덮어쓰는 구조라 축소 가드가 필요했고,
    그 가드는 정기 스케줄에서 원리적으로 동작할 수 없었다(공고당 조회가 2회뿐이라
    관측 경과가 24h 한 값 → 어떤 시효도 "항상 채택" 아니면 "영구 보류").
    병합이라 **삭제 경로가 없으므로**, 잘못 들어간 행은 코드로 복구되지 않는다 —
    그래서 `company` 는 파서에서 `.strip()` 으로 정규화하고 UNIQUE 를 건다.
    """
