...
- **체험 라이프사이클 시퀀스**: 구현 완료(2026-08-02). D0 웰컴(거래) / D1 매칭·D3 익스텐션·D7 요약(광고,
  `trial.send_onboarding_sequence` 매일 10:10) / D-3·D-1 만료 고지(거래) / 만료 후 윈백 할인(광고).
  **카테고리 배치가 이 시퀀스의 전부다** — 만료 '사실'은 거래라 전원에게 가고, 할인은 광고라
  동의·확인을 마친 회원에게만 간다. 그래서 미동의자도 체험이 끝난다는 사실은 반드시 받는다.
  윈백 금액·유예일은 `schemas/subscription.py` 상수에서 파생시킨다(가격 개편 때 메일이 거짓말하지 않도록).
- **가입 더블 옵트인**: `/signup` 도 이메일 소유를 확인하지 않으므로 `confirmed=False` + 확인 메일.
  확인 엔드포인트는 리드·회원 공용 `endpoints/optin.py`(`GET /optin/status` 조회 / `POST /optin` 확정).
  옵트인 토큰과 수신거부 토큰은 **용도가 분리**돼 서로 쓸 수 없다 — 섞이면 유출된 해지 링크로 재구독이 된다.
- **아직 안 한 것**: 알림톡 어댑터(채널 개설 대기).
