...
## 3. 실행 전 확인할 미해결 (probe 1회로 해결)
- [ ] 용역·물품 `bsnsDivCd` 코드 (1페이지 테스트 호출로 확정)
- [ ] getDataSetOpnStdScsbidInfo 응답 스키마가 카테고리 무관 동일한지(_parse 재사용 가능?)
- [ ] 용역 적격심사 변동 하한율 소스(계약유형별) — 없으면 용역은 "타당성 확인" 수준으로 한정
- [ ] `category` 컬럼 마이그레이션
