...
동의·발송·억제 3층이 갖춰졌으므로 퍼널 ③(육성)을 실제로 켰다. 접촉은 **확인 1통 + 광고 2종**뿐이다.

| 시점 | 템플릿 | 성격 | 멱등 키 |
|---|---|---|---|
| 캡처 직후(동기) | `lead_optin_confirm` | **거래**(확인 요청) | `lead_optin_confirm:email:{sha1}` |
| 확인 링크 클릭 후 | `lead_welcome` | 광고 | `lead_welcome:email:{sha1}` |
| 매주 화 08:00 KST | `lead_new_matches` | 광고 | `lead_new_matches:email:{sha1}:{YYYY}W{주차}` |
...
1. **더블 옵트인.** `/leads/capture` 는 인증도 이메일 소유 확인도 없는 공개 폼이다. 요청 본문의
   `marketing_consent: true` 만 믿고 광고를 보내면, 남의 주소를 적어 넣은 요청이 그대로 **제3자에게
   보내는 광고**가 된다. 우리가 가진 증적은 제출자의 IP·UA 일 뿐 주소 소유자의 동의가 아니다.
   그래서 캡처 시점에는 `marketing_confirmed_at` 을 비워 두고(= `sendable_filter` 가 막는다),
   확인 링크를 누른 뒤에야 광고 대상이 된다. 확인 메일 자체는 광고를 담지 않는다 — 담는 순간
   미확인 주소로 보낸 광고물이 되어 도입 이유가 사라진다.
2. **확인은 POST 로만.** 수신거부와 같은 규칙이다. GET 으로 확정하면 메일 서버·보안 스캐너의
   링크 프리페치가 사용자 대신 확인을 눌러 더블 옵트인이 무의미해진다.
3. **멱등의 주체는 행이 아니라 수신자.** 캡처는 이메일 upsert 없이 매번 새 `Lead` 행을 만든다.
   키를 `lead.id` 로 잡으면 같은 사람이 재진단·더블클릭할 때마다 키가 갈라져 같은 메일이 여러 통
   나간다. 그래서 키를 이메일 정규화값 해시로 잡고, 배치도 수신자당 1건만 뽑는다.
4. **주 1회.** 반송률 5%·불만율 0.1% 초과 시 AWS 가 계정 발송을 정지시키고, 그러면 광고뿐 아니라
   **거래 메일까지 막힌다.** 리드 모수가 작은 지금은 빈도를 올려 얻을 것보다 잃을 것이 크다.
   빈도는 나중에 올리기 쉽지만 스팸으로 인식된 신뢰는 되돌리기 어렵다.
5. **실패는 언제나 1건에 갇힌다.** 리드 1건의 데이터 결함(제목에 섞인 개행 등)이 배치를 통째로
   끊으면 매주 같은 지점에서 전원이 조용히 메일을 못 받는다(beat 는 실패를 알리지 않는다).
   그래서 ① 입력 시점에 제어문자를 정제하고 ② `mailer` 가 조립 실패도 `MailerError` 로 변환하며
   ③ 배치는 리드 단위로 예외를 가둔다. 실패는 원장에 `failed` + **키를 놓은 상태**로 남아 재시도
   여지를 남긴다(선점만 하고 멈춘 `sending` 유령행이 재발송을 영구히 막는 일이 없도록).

⚠️ **소급 발송 없음**: 대상은 `sendable_filter` 통과분뿐이다. 2026-07-30 이전 캡처분은 증적이
없어 자동으로 빠진다(현재 `leads` 0건이라 실질 모수는 새로 들어오는 리드부터).

⚠️ **`can_send_marketing` 은 `marketing_confirmed_at` 만 본다.** 예전엔 이 값이 없으면
`marketing_consent_at` 으로 폴백했는데, 그러면 ① 확인 대기 상태가 발송 가능으로 새고 ② SQL
판본(`sendable_filter`)은 이 컬럼을 필수로 요구하므로 단건 판정과 갈라진다. 판정이 두 갈래면
언젠가 반드시 위법 발송 쪽으로 갈라진다.
