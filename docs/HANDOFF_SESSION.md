...
> 작성 2026-08-08 · 갱신 2026-08-19. 최신 master `29a35db` · 운영 배포 `44d2c3c`.
> ⚠️ **에이전트 둘이 같은 레포에서 동시에 움직였다** — 아래 §0-1 을 먼저 읽을 것.
...
> **2026-08-19 현재 초점**: 모의투찰은 **코드가 닫혔고 관찰 대기**다. 08-14 에
> 전략 파라미터화를 `product_v1` → `ratio_pinned_v2` 로 전환(운영 active
> `v20260814_050218_d67a75`)했고, 그 뒤 드러난 결함 넷을 전부 수습해 배포했다
> (#113·#114·#115·#122·#124). 실사용자 추천가는 전환 시점에 한 번만 바뀌었고
> 이후 변동 없다. **G-A 55.22%**(1,699/3,077, OBSERVING) · **G-B 70/400** 이라
> 성능 우열 해석은 계속 금지다(§0.4).
>
> ⚠️ **미해결 하나**: Codex 가 돌린 참가자 복구가 **완주했으나 판정 실패**했다
> (`EXIT=1`, mismatch 4.71% · null_rank 46.38%). 크롤·백필 오류는 0건이라 실행
> 문제가 아니라 **회복이 안 된 것**이다. 로그는 호스트 `/tmp/bideasy_mock_recovery_44d2c3c.log`.
> Codex 세션 `019fef5e` 가 복귀하면 이어받을 몫이다(§0-1).
>
> **Claude 트랙**은 유입으로 옮겼다 — 허가형 메시지 검증을 가동 직전까지 만들었고
> (#130) **아직 켜지 않았다**. 남은 것은 모집 채널 승인 하나다.

---

## 0-1. ⚠️ 교차 인계 — Codex 세션 `019fef5e` 가 돌아오면 읽을 것 (2026-08-18)

Codex 세션 하나가 08-11부터 모의투찰을 맡아 왔고, 08-18 16:06 KST 에 **사용량
제한(`Your access t…`)으로 중단**됐다. 그 사이 Claude Code 세션이 같은 레포에서
작업했다. **서로 모르는 채로 겹쳤고, 실제로 한 번 충돌했다.**

### Codex 가 모르는 사실 셋

| | |
|---|---|
| **PR #124 를 Claude 가 머지·배포했다** | Codex 가 **draft** 로 둔 것을 "승인 대기"로 읽고 ready 전환 후 머지(`44d2c3c`, 06:20 UTC). CI 4종 green 이었고 내용도 완결돼 있었지만, **draft 는 "아직 준비 안 됨" 신호였을 수 있다** |
| **그 배포가 복구 프로세스를 죽였다** | 06:02 UTC 에 Codex 가 띄운 참가자 복구가 18분 진행 중이었는데, 06:20 배포가 `app` 컨테이너를 재생성하며 프로세스와 컨테이너 `/tmp` 로그를 함께 날렸다. Codex 가 06:23 에 재시작했다(현재 로그 `/tmp/bideasy_mock_recovery_44d2c3c.log`, 호스트) |
| **#124 의 효과는 운영에서 확인됨** | `LoadStats(loaded=11244, static=4848, db_contributed=6396, excluded=719, kept=10525)` — 제외 수가 다시 보인다. #122 배포 이후 이 값이 관리자 배너·G2 산출물에서 0 으로 거짓 기록되고 있었다 |

### 이 사고에서 남길 것

**배포는 컨테이너를 재생성한다.** 컨테이너 안에서 도는 장기 프로세스(복구·백필)는
배포와 함께 죽고, 컨테이너 `/tmp` 의 로그도 사라진다. 다른 에이전트가 돌리고 있는
작업이 있는지 **배포 전에 `ps aux | grep -E "repair|backfill"` 로 확인**할 것.
장기 작업은 호스트 `/tmp` 에 로그를 남기는 쪽이 안전하다(06:23 재시작분이 그렇게 했다).

**draft PR 은 건드리지 않는다.** CI 가 green 이어도 draft 는 저자가 붙인 상태다.
머지가 필요하면 저자 세션이 돌아올 때까지 기다리거나 사용자에게 확인한다.

### Claude 가 그 사이 한 일 (모의투찰 밖)

`chore/message-test-preflight` → **PR #130 머지**(`571d8dc`). 허가형 메시지 검증의
가동 전 점검이다 — 구현 349줄에 테스트가 **0건**이던 것을 24건 채우고, 준비돼 있던
화면이 **물품 구매 공고**여서 전기공사 공고로 교체했다. **아무것도 켜지 않았다**
(`MESSAGE_TEST_ENABLED` 기본 `False` 유지). 상세 = `docs/MESSAGE_TEST_PREFLIGHT.md`.
...
(테스트 **1,096건** green · 그런데 **리드 1명 / 회원 2명** — 08-08 스냅샷과 동일,
최근 7일 신규 리드 **0건**. 그 열흘 동안 모의투찰 알고리즘을 다듬었다.)

그래서 다음 작업의 우선순위는 이렇게 갈립니다:

| 순위 | 무엇 | 왜 |
|---|---|---|
| 1 | **모집 채널 승인 → 메시지 검증 가동** | 리드 1명이 진짜 병목이다. 장치는 다 만들어졌고(#130) 켜기만 남았다 — 운영진 승인 커뮤니티·기존 동의 관계 한정(§5-C) |
| 2 | **등수축 건강도 미회복** | 복구가 완주했으나 `EXIT=1`. **Codex `019fef5e` 몫**(§0-1) |
| 3 | G-A 재판정 (8월 말) | 관찰 창 28일 완료 시점. 60% 미달이면 10월 초 kill 창 |
| 4 | AWS 루트 키 폐기 | 서버 무관 확정(오경보)이나 키는 여전히 Active·맥북 평문 (§3) |
| 5 | 유입 효과 판정 | 색인 표면 **10,943 URL**(07-30 대비 5배). 서치콘솔·GA4 는 사람이 봐야 한다 |

### 모의투찰 수치를 읽는 법 (반복해서 틀린 지점)

`mock_bids` 는 한 공고에 **5 arm 행**이 생긴다 — 행 수를 표본 수로 세지 말 것
(2026-08-10 에 "SCORED 2,275건이므로 G2 400건 충족"으로 오판한 적이 있다).
그리고 성능 집계는 **기초금액 일관성 통과분만**(사정률 0.94~1.06) 쓰고 제외 수를
함께 보고한다. 2026-08-18 실측: 등록 3,077공고 / 채점 1,699 / **G-A 55.22%**.
상세·게이트 정의는 `docs/MOCK_BIDDING_DESIGN.md` §9.
...
cd backend && pytest                          # 1,096건 통과해야 정상 (2026-08-19)
...
> 실측 참고: Apple Silicon 에서 `pytest` 1,096건이 **30초**(2026-08-19).
> ⚠️ `Pillow` 가 없으면 크리에이티브 테스트 3건이 **수집 단계에서** 깨진다 —
> `pip install -r requirements.txt` 를 최신으로 다시 돌릴 것.
...
## 4-1. 2026-08-14~19 에 일어난 일 (PR 9건)

### 모의투찰 — R1 전환과 그 여파

`adjustment` 는 **사정률 예측으로 적합된 적이 없었다.** 목적함수가 `(adj, margin)`
을 **곱으로만** 봐서 식별되지 않고 margin 과 임의로 교환된 값이었다(#112). 그걸
사정률 중심에 고정하고 margin 1축으로 바꾼 것이 **R1**(#113)이고, 전환은 승격
가드(`MAX_PARAM_JUMP=1.0`)에 걸리므로 **루프 밖에서 1회 실행**했다
(`scripts/migrate_strategy_parametrization.py`). 가드는 손대지 않았다.

**R1 이 파라미터에 의미를 부여하자, 그 값을 만들던 경로의 품질 문제가 줄줄이
드러났다** — 이게 이번 주의 핵심이다:

| PR | 드러난 것 |
|---|---|
| #114 | 얕은 표본의 **부모 중앙값이 예측을 지배**(방법 전체 n=2 → adj +1.3) |
| #115 | **곱 보존은 A값이 0 일 때만 안전**하다 — 가격식 둘째 항이 방향을 뒤집는다 |
| #122 | 자가보정이 **오염 데이터로 적합**하고 있었다(개찰 원장 10.2%가 사정률 1.1배) |
| #124 | #122 가 넣은 필터가 **호출자의 제외 수 계산을 0 으로 만들었다**(회귀) |

⚠️ **셋은 테스트가 전부 green 인 상태에서, 실행해 숫자를 봐야 나왔다.** 특히
#115 는 "곱을 보존했는가"는 잘 쟀는데 **정작 재야 할 것이 사용자가 받는 가격**
이었다 — 검증 대상 자체를 잘못 잡은 경우다.

### 사정률 오차의 하한은 제도가 정한다 (실측)

예정가격은 예비가격 15개 중 4개를 추첨해 평균낸 값이라 분산이 이론적으로 정해지고,
실측이 **이론값의 85~87%** 로 들어맞는다(±3%: σ 0.00752 vs 0.00866 / ±2%: 0.00488
vs 0.00577). **모델을 바꿔 줄일 수 있는 오차가 아니다.** 유일한 개선 경로는 ±2%
구분(σ 35% 감소)인데, 보류 사유였던 커버리지가 12.1% → **37.6%** 로 올랐다(R2).
⏸ 다만 그 개선이 향하는 곳(G-C)이 G-B 에 막혀 있어 **기다릴수록 조건이 좋아진다.**

### 유입 — 메시지 검증을 가동 직전까지

허가형 5초 메시지 A/B 테스트는 **구현 349줄에 테스트가 0건**이었다("배포·외부 모집
전 검수 중"이라면서 검수 장치가 없었다). 24건을 채우고 안전장치를 **실제로 껐다
켜서** 확인했다(#130). 준비돼 있던 화면은 **물품 구매 공고**여서 전기공사 공고로
교체했다 — 그대로 켰으면 전기공사 업체에게 서버 구매 화면을 보여주고 "메시지가 안
통한다"고 결론 낼 뻔했다. **메시지가 아니라 화면이 틀린 건데도.**

절차·판정 기준 = `docs/MESSAGE_TEST_PREFLIGHT.md`. **아직 켜지 않았다.**
...
```bash
cd backend && pytest              # 1,096건 기준 (2026-08-19)
