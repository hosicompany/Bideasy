...
> 작성 2026-08-08. 기준: master `99199f9` (#90) / 운영 `27439db`.
...
| 순위 | 무엇 | 왜 |
|---|---|---|
| 1 | 🚨 AWS 루트 키 교체 | 보안. 지금 운영이 루트 키로 돈다 (§3) |
| 2 | #90 배포 | master 에 있는데 운영에 없다 (§6) |
...
```
C:\dev\bideasy-suite\            ← 작업 루트 (4개 레포가 형제 폴더)
...
| 레포 | 브랜치 | HEAD | 비고 |
|---|---|---|---|
| Bideasy | `master` | `99199f9` (#90) | ⚠️ **운영은 `27439db`** — #90(누적 개찰 통계 S1)이 **미배포**. 마이그레이션 `f7c4a2e18b53` 도 미적용 |
...
## 3. 🚨 지금 열려 있는 위험 — 먼저 읽을 것

### AWS 루트 액세스 키가 운영에서 쓰이고 있다

2026-08-08 실측: `bideasy_app`·`bideasy_celery`·`bideasy_celery_beat` 의 `AWS_ACCESS_KEY_ID` 가
**AWS 루트 계정 액세스 키**(`AKIAY5OAWO54BX…`)입니다. SES 발송 전부가 이 키로 나갑니다.
루트 키는 MFA 로도 권한 축소가 불가능한 전권이라, 유출되면 계정 전체를 잃습니다.

**⛔ 절대 먼저 지우지 마세요.** 지우면 광고 메일뿐 아니라 체험 만료 고지·수신거부 처리 결과 통지 같은
**정보통신망법상 보내야 하는 메일까지 전면 중단**됩니다.

순서는 **발급 → 주입 → 배포·검증 → 그 다음 비활성화 → 관찰 → 삭제**.
전체 절차·명령 = **`docs/SECRET_ROTATION.md` §3-1**.

진행 상황: 2026-08-05 에 사전 백업(`~/env.production.bak-20260805-rootkey`)까지만 하고 중단.

> **에이전트에게**: 이 작업에서 비밀키 **값**을 채팅·파일·커밋에 남기지 마세요.
> 읽어도 되는 것은 **키 이름·문자 길이·ID 앞자리**까지입니다.
> 키 발급과 서버 주입은 사람이 합니다(🔴 등급). 백업·배포·검증·문서 정정은 에이전트가 해도 됩니다.

### 왜 이걸 아무도 몰랐나 (반복 금지)

문서 **6곳**이 "서버는 발송 전용 IAM `bideasy-ses-sender` 를 쓴다"고 적고 있었습니다. 전부 틀렸습니다.
근거는 **로컬 PC 점검뿐**이었고 서버는 아무도 열어보지 않았습니다.
→ **"무관하다"는 판단에는 실측 근거를 붙일 것.** 이번에 6곳 전부 정정했습니다.
...
⚠️ **2026-08-08 현재 master 에 미배포 커밋이 하나 있습니다** — #90(`99199f9`, 누적 개찰 통계 S1,
마이그레이션 `f7c4a2e18b53` 포함). 운영은 `27439db` / `b6f1d3a48c27`.
루트 키 교체(§3)와 함께 배포하면 재기동이 한 번으로 끝납니다.
