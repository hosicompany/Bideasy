...
> 작성 2026-08-08 · 갱신 2026-08-16. 현재 작업선: `feat/blog-knowledge-autopublish` (최신 master `b130563` 위, 로컬 변경 미push·미배포) / 운영 `b130563`.
...
> **2026-08-16 현재 세션 초점**: 블로그 K-트랙 자동 포스팅 Phase 2. 완결된 PASS/WARN만 48시간 유예 예약하고 FAIL·검사 생략은 사람 호출, 발행 직전 재검증·편집/force 재생성 시 예약 무효화·Postgres 중복 실행 선점·깨진 히어로 제거까지 구현했다. backend 979 passed·Ruff clean. 운영에는 아직 반영하지 않았으며, K1~K8 발행·K9 FAIL draft·예약 0이라 배포 후 다음 수요일 대상은 K10이다. 다음 단계는 로컬 커밋 정리 → 사람의 push 승인 → PR 리뷰 → 코드 머지·배포 승인이다.
